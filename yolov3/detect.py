...
def video_unpacker(path, delete_after=False):
    ...
    for image in range(lenght):
        ...
        cv2.imwrite(os.path.join('frames_in/', name), frame)
...
def video_packer(path_in, path_out, delete_after=False):
    ...
    out = cv2.VideoWriter(os.path.join(path_out, 'video_out.mp4'), cv2.VideoWriter_fourcc(*'DIVX'), 15, size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--video_file", type=str, default="video.mp4", help="path to video")
    ...
    video_unpacker(opt.video_file)
    model.eval()  # Set in evaluation mode

    dataloader = DataLoader(
        ImageFolder('frames_in', transform= \
    ...
    for img_i, (path, detections) in enumerate(zip(imgs, img_detections)):
        ...
        output_path = os.path.join("frames_out", f"{filename}.png")
        plt.savefig(output_path, bbox_inches="tight", pad_inches=0.0)
        plt.close()

video_packer('frames_out', '')